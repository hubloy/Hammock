...
export default class EditRuleModal extends Component {
    ...
    async componentDidMount() {
        ...
        this.handleTypeSelect( null, rule.object_type );
        this.setState({ checked: rule.status == 'enabled' });
    }

	notify(message,type) {
		toast[type](message, {toastId: 'rule-edit-toast'});
	}
	...
	load_items = async ( type ) => {
		this.fetchWP.get( 'rules/items/' + type + '?id=' + this.props.rule.rule_id )
			.then( (json) => {
				this.setState({
					items : json,
					loading : false
	...
	load_memberships = async () => {
		this.fetchWP.get( 'rules/memberships?id=' + this.props.rule.rule_id )
			.then( (json) => {
				this.setState({
					membership : json,
					membership_loading : false
