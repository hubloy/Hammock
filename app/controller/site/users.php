...
class Users extends Controller {
	...
	public function manage_users_columns( $columns ) {
		...
		if ( $this->settings->get_general_setting( 'account_verification' ) === 1 ) {
			$membership_column = array( 'membership' => __( 'Is Member' , 'hammock' ), 'verified' => __( 'Verified' , 'hammock' ) );
		}
	...
	public function handle_verify_bulk_message() {
		if ( isset ( $_REQUEST['_hammock_approved'] ) ) {
			...
			<?php
		} else if ( isset ( $_REQUEST['_hammock_disapproved'] ) ) {
			...
			<?php
		} else if ( isset ( $_REQUEST['_hammock_resend'] ) ) {
