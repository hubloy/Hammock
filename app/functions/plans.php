 ...
 * @return array
 */
function hubloy_membership_get_plans_to_join() {
 ...
 * )
 */
function hubloy_membership_can_user_join_plan( $plan_id ) {
	if ( is_user_logged_in() ) {
		if ( ! hubloy_membership_current_user_can_subscribe() ) {
			...
			$has_plan = hubloy_membership_current_user_has_plan( $plan_id );
			if ( $has_plan ) {
				return array(
					'status'  => false,
 ...
 * @return bool
 */
function hubloy_membership_current_user_has_plan( $plan_id ) {
 ...
 * @return object|bool
 */
function hubloy_membership_get_plan_by_id( $plan_id ) {
	$service = new \HubloyMembership\Services\Memberships();
 ...
 * @return object|bool
 */
function hubloy_membership_get_member_plan_subscription( $user_id, $plan_id ) {
	$member_service = new \HubloyMembership\Services\Members();
	$member         = $member_service->get_member_by_user_id( $user_id );
