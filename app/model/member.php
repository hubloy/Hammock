...
class Member {
	...
	public function drop_plan( $old_membership_id ) {
		...
		if ( $old_plan ) {
			// Delete
			do_action( 'hubloy_membership_member_before_remove_old_plan', $old_plan, $old_membership_id, $this );

			$old_plan->delete();

			do_action( 'hubloy_membership_member_after_remove_old_plan', $old_membership_id, $this );
	...
	public function drop_all_plans() {
		...
		do_action( 'hubloy_membership_member_before_drop_all_plans', $plans, $this );
		...
		do_action( 'hubloy_membership_member_after_drop_all_plans', $plans );
	}
	...
	public function delete() {
		...
		do_action( 'hubloy_membership_member_before_delete_member', $this );
		...
		do_action( 'hubloy_membership_member_after_delete_member', $this->id );
	...
	public function add_plan( $new_membership_id, $args = array() ) {
		...
		if ( $membership->id > 0 ) {
			do_action( 'hubloy_membership_member_before_add_plan', $membership, $this );
			...
			if ( ! $old_plan ) {
				...
				$new_plan->enabled       = apply_filters( 'hubloy_membership_new_plan_enabled', true, $membership );
				$new_plan->status        = apply_filters( 'hubloy_membership_new_plan_status', Members::STATUS_PENDING, $membership );
				...
				do_action( 'hubloy_membership_member_after_add_plan', $membership, $new_plan, $this );
				return $new_plan;
			} else {
				if ( isset( $args['status'] ) ) {
					if ( $args['status'] == Members::STATUS_TRIAL ) {
				...
				$old_plan->save();
				do_action( 'hubloy_membership_member_after_update_plan', $membership, $old_plan, $this );
				return $old_plan;
			}
		}
		return false;
	...
	public function move_plan( $old_membership_id, $new_membership_id ) {
		...
		do_action( 'hubloy_membership_member_before_move_plan', $old_membership_id, $new_membership_id, $this );
		$this->drop_plan( $old_membership_id );
		do_action( 'hubloy_membership_member_after_move_plan', $old_membership_id, $new_membership_id, $this );
	...
	public function edit_url() {
		return admin_url( 'admin.php?page=hubloy_membership-members#/member/' . $this->id );
	}
	...
	public function can_trial( $membership_id ) {
		if ( hubloy_membership_user_can_subscribe( $this->user_id ) ) {
	...
	public function to_html() {
		return apply_filters(
			'hubloy_membership_member_to_html',
			array(
				'id'            => $this->id,
