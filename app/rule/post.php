...
class Post extends Rule {
	...
	public function protect_content() {
		...
		do_action(
			'hubloy_membership_protect_post_content',
			$this
		);
	}
	...
	public function manage_posts_clauses( $clauses, $wp_query ) {
		...
		$clauses['where'] .= apply_filters( 'hubloy_membership_post_rule_manage_posts_clauses', '' );
		return $clauses;
	}
	...
	public function exclude_restricted_pages( $pages ) {
		...
		if ( 'hide' === $protection_level ) {
			...
			foreach ( $pages as $index => $page ) {
				$restricted = hubloy_membership_is_post_protected( $page );
				if ( $restricted ) {
					unset( $pages[ $index ] );
				}
			}
	...
	public function exclude_restricted_content_comments( $posts, $query ) {
		...
		if ( ! empty( $query->comment_count ) && is_comment_feed() ) {
			...
			foreach ( $query->comments as $key => $comment ) {
				...
				$restricted = hubloy_membership_is_post_protected( $post_id );
	...
	public function exclude_restricted_comments( $comment_query ) {
		 ...
		 * @since 1.0.0
		 */
		$restrictable_comment_types = apply_filters( 'hubloy_membership_post_rule_exclude_restricted_comments_types', array( '', 'trackback', 'pingback', 'review', 'contribution_comment' ) );
		...
		if ( isset( $comment_query->query_vars['type'] ) && in_array( $comment_query->query_vars['type'], $restrictable_comment_types, true ) ) {
			...
			$restricted = hubloy_membership_is_post_protected( $post );
	...
	public function restrict_post( $post ) {
		...
		if ( ! in_array( $post->ID, $this->content_restricted, false ) && hubloy_membership_is_post_protected( $post->ID ) ) {
	...
	public function rss_enclosure_content( $enclosure ) {
		...
		if ( $enclosure && $post ) {
			$can_view = hubloy_membership_is_post_protected( $post );
		}
