 ...
 *
 * This template can be overridden by copying it to yourtheme/memberships-by-hubloy/account/access/login-form.php.
 *
...
<div class="hubloy_membership-account-access-login <?php echo esc_attr( $extra_classes ); ?>">
	<h4><?php _e( 'Login to your account', 'memberships-by-hubloy' ); ?></h4>
	<form name="loginform" class="hubloy_membership-ajax-form" id="loginform">
		...
		<p class="login-password">
			<label for="user_pass"><?php _e( 'Password', 'memberships-by-hubloy' ); ?></label>
			<input type="password" name="user_pass" id="user_pass" class="input password-input" value="" size="20" required />
		</p>
		<p class="forgetmenot">
			<input name="rememberme" type="checkbox" id="rememberme" value="forever" /> <label for="rememberme"><?php esc_html_e( 'Remember Me', 'memberships-by-hubloy' ); ?></label>
		</p>
		<?php
			do_action( 'hubloy_membership_after_account_login_form_before_submit_button' );
		?>
		<p class="submit">
			<button name="wp-submit" id="wp-submit" class="button button-primary button-large"><?php esc_attr_e( 'Log In', 'memberships-by-hubloy' ); ?></button>
		</p>
		<?php
