 ...
 *
 * This template can be overridden by copying it to yourtheme/hubloy-membership/account/plan/list/view-plan.php.
 *
...
?>
<td class="hubloy-membership-account-membership-plan--title"><?php echo esc_html( $membership->name ); ?></td>
<td class="hubloy-membership-account-membership-plan--status"><?php echo esc_html( $plan->status_detail ); ?></td>
<td class="hubloy-membership-account-membership-plan--price">
	...
	if ( $membership->trial_enabled && $member->can_trial( $membership->id ) ) {
		...
		if ( ! $member->has_subscribed_before( $membership->id ) && $membership->is_recurring() && $membership->signup_price > 0 ) {
			...
			echo esc_html( hubloy-membership_format_currency( $membership->price ) );
			?>
			<span class="hubloy-membership-account-membership-plan--price-description">
			<?php
...
<td class="hubloy-membership-account-membership-plan--payment">
	<a class="hubloy-membership-account-membership-plan--price--can-join" href="<?php echo esc_url( hubloy-membership_get_account_page_links( 'view-plan', $membership->membership_id ) ); ?>">
		<?php
		if ( ! $plan->is_active() ) {
			esc_html_e( 'Inactive', 'hubloy-membership' );
		} else {
			esc_html_e( 'Active', 'hubloy-membership' );
		}
		?>
	</a>
</td>
